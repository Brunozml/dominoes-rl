...
class SingleSideGame:
    ...
    :var moves: a list of the moves that have been played. Moves are
                ...
                played (True for left, False for right). In this version of Game, 
                MOVES WILL ONLY BE EXECUTED TO THE LEFT
    ...
    def _update_valid_moves(self):
        ...
        for d in self.hands[self.turn]:
            ...
            # comment out the following to only allow for one side
...
game = SingleSideGame.new(starting_domino= dominoes.Domino(6,6))
...
def single_game(player_settings):
    ...
    single_game = SingleSideGame.new(starting_domino= dominoes.Domino(6,6))
    ...
    while single_game.result is None: 
            ...
            player = player_settings[single_game.turn]
